...
export class errorCheck implements Command {
    name: string = "오류체크";
    description?: string | undefined = "오류체크 명령어";
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<any> {
        try {
            await createRaidStatus(interaction.user.id, interaction);
            await interaction.reply({ content: "ㅁㅁ", ephemeral: true }).then((msg) => setTimeout(() => msg.delete()));
        } catch (err) {
            await interaction.reply({ content: "오류", ephemeral: true }).then((msg) => setTimeout(() => msg.delete(), 5000));
        }
    }
